...
class TestConsul(unittest.TestCase):
    ...
    def test_take_leader(self):
        ...
        with patch('patroni.dcs.consul.get_monotonic_time', Mock(side_effect=[0, 0, 0, 100, 100, 100])), \
                patch('patroni.utils.get_monotonic_time', Mock(side_effect=[0, 0, 0, 100, 100, 100])):
            self.assertFalse(self.c.take_leader())
    ...
    def test_update_leader(self, mock_renew):
        ...
        with patch.object(KV, 'delete', Mock(return_value=True)):
            ...
            with patch('patroni.dcs.consul.get_monotonic_time', mock_time), \
                    patch('patroni.utils.get_monotonic_time', mock_time):
                self.assertRaises(ConsulError, self.c.update_leader, cluster, 12345)
        with patch('patroni.dcs.consul.get_monotonic_time', Mock(side_effect=[0, 100, 200, 300])), \
    ...
    def test_sync_state(self):
        ...
        with patch('patroni.utils.get_monotonic_time', Mock(side_effect=[1, 100, 1000])):
