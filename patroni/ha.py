...
class Failsafe(object):
    ...
    def update(self, data: Dict[str, Any]) -> None:
        ...
        """
        with self._lock:
            self._last_update = get_monotonic_time()
    ...
    def leader(self) -> Optional[Leader]:
        ...
        with self._lock:
            if self._last_update + self._dcs.ttl > get_monotonic_time():
    ...
    def is_active(self) -> bool:
        ...
        with self._lock:
            return self._last_update + self._dcs.ttl > get_monotonic_time()
...
class Ha(object):
    ...
    def is_leader(self) -> bool:
        ...
        with self._leader_expiry_lock:
            return self._leader_expiry > get_monotonic_time()
    ...
    def set_is_leader(self, value: bool) -> None:
        ...
        """
        with self._leader_expiry_lock:
            self._leader_expiry = get_monotonic_time() + self.dcs.ttl if value else 0
            if not value:
                self._promote_timestamp = 0
    ...
    def _handle_crash_recovery(self) -> Optional[str]:
        if self._crash_recovery_started == 0 and (self.cluster.is_unlocked() or self._rewind.can_rewind):
            self._crash_recovery_started = get_monotonic_time()
    ...
    def _process_quorum_replication(self) -> None:
        ...
        def _check_timeout(offset: float = 0) -> bool:
            return get_monotonic_time() - start_time + offset >= self.dcs.loop_wait
    ...
    def process_sync_replication(self) -> None:
        ...
        if self.is_quorum_commit_mode():
            ...
            if self.state_handler.is_primary():
                if self._promote_timestamp == 0 or get_monotonic_time() - self._promote_timestamp > self.dcs.loop_wait:
                    self._process_quorum_replication()
                if self._promote_timestamp == 0:
                    self._promote_timestamp = get_monotonic_time()
        ...
        if not self.is_paused():
            ...
            with self._async_response:
                if self._async_response.result is False:
                    ...
                    self._released_leader_key_timestamp = get_monotonic_time()
                    self.release_leader_key_voluntarily()
    ...
    def is_healthiest_node(self) -> bool:
        ...
        """
        if get_monotonic_time() - self._released_leader_key_timestamp < self.dcs.ttl:
    ...
    def process_unhealthy_cluster(self) -> str:
        ...
        if not self.is_paused() and not self.is_standby_cluster() and \
            ...
            if self._primary_race_backoff_timestamp == 0:
                self._primary_race_backoff_timestamp = get_monotonic_time()
            time_left = self._primary_race_backoff_timestamp + global_config.primary_race_backoff - get_monotonic_time()
    ...
    def handle_long_action_in_progress(self) -> str:
        ...
        if self.has_lock() and self.update_lock():
            if self._async_executor.scheduled_action == 'doing crash recovery in a single user mode':
                time_left = global_config.primary_start_timeout - (get_monotonic_time() - self._crash_recovery_started)
        ...
        if not self.is_paused() and self.state_handler.is_running():
            if self.state_handler.is_primary():
                if self.is_failsafe_mode() and self.check_failsafe_topology():
                    self.set_is_leader(True)
                    self._failsafe.set_is_active(get_monotonic_time())
                    self.watchdog.keepalive()
